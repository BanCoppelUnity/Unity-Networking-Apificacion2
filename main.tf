...
locals {
  ...
  private_route_table_definition = [
    for subnet_name, subnet_values in local.private_subnets :
    subnet_name
...
# Crea una VPC utilizando el módulo 'Unity-VPC-module' sin un Internet Gateway asociado
module "vpc_module" {
...
module "subnet_module" {
  ...
  tags = can(regex("eks", each.key)) ? merge({
    ...
    "kubernetes.io/cluster/bcpl-eks-dev-apificacion" = "owned"
...
resource "aws_default_route_table" "main_route_table" {
  ...
  # Define las etiquetas para el NAT gateway, incluyendo ñas etiquetas 'Name' y 'Service Name'
  tags = merge(local.tags, {
